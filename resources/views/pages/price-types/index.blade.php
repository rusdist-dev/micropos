<x-layouts.app title="Tipe Harga">
    <div
        x-data="{
            loading: true,
            error: null,
            ...
            async load() {
                ...
                try {
                    ...
                    const res = await window.api.get('/api/price-types?' + params.toString());
            ...
            async doDelete() {
                try {
                    const res = await window.api.delete('/api/price-types/' + this.deleting.id);
                    this.$store.toasts.success(res.message || 'Tipe harga dihapus');
    ...
    >
        <x-ui.alert variant="info" class="mb-4">
            Tipe harga bersifat dinamis. Setiap produk dapat memiliki sebagian atau seluruh tipe harga ini.
            Di kasir, katalog hanya menampilkan produk yang memiliki harga untuk tipe yang dipilih.
        </x-ui.alert>

        <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="sm:w-80"><x-ui.search-input placeholder="Cari nama atau kode..." model="search" /></div>
            <x-ui.button :href="route('price-types.create')" icon="plus">Tambah Tipe Harga</x-ui.button>
        </div>
        ...
        <div x-show="!loading && !error && items.length > 0">
            <x-ui.table>
                <x-slot:head>
                    <x-ui.th align="center">Urutan</x-ui.th>
                ...
                <template x-for="t in items" :key="t.id">
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-1 text-center text-sm text-gray-500" x-text="t.sort_order"></td>
                        <td class="px-4 py-1">
                            <div class="flex items-center gap-3">
                                <span class="flex h-9 w-9 items-center justify-center rounded-lg bg-primary-50 text-primary-600"><x-heroicon-o-tag class="h-5 w-5" /></span>
                                <span class="text-sm font-medium text-gray-800" x-text="t.name"></span>
                            </div>
                        </td>
                        <td class="px-4 py-1"><span class="rounded bg-gray-100 px-2 py-0.5 font-mono text-xs text-gray-600" x-text="t.code"></span></td>
                        ...
                        <td class="px-4 py-1">
                            <div class="flex items-center justify-end gap-1">
                                <a :href="'{{ url('price-types') }}/' + t.id + '/edit'" class="rounded-md p-1.5 text-gray-400 transition hover:bg-primary-50 hover:text-primary-600" title="Edit"><x-heroicon-o-pencil-square class="h-4 w-4" /></a>
        ...
        <x-ui.modal title="Hapus Tipe Harga" size="sm" show="showDelete">
            <p class="text-sm text-gray-600">Hapus tipe harga <span class="font-semibold" x-text="deleting?.name"></span>? Tipe yang sedang dipakai produk tidak dapat dihapus.</p>
            <x-slot:footer>
