<x-layouts.app title="Pemasok">
        ...
        x-data="{
            ...
            async load() {
                ...
                try {
                    ...
                    const res = await window.api.get('/api/suppliers?' + params.toString());
            ...
            async doDelete() {
                try {
                    const res = await window.api.delete('/api/suppliers/' + this.deleting.id);
                    this.$store.toasts.success(res.message || 'Pemasok dihapus');
    ...
    >
        <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <x-ui.button :href="route('suppliers.create')" icon="plus">Tambah Pemasok</x-ui.button>
        </div>

        <div x-show="loading" class="py-10"><x-ui.loading-spinner size="lg" label="Memuat pemasok..." /></div>
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-1">
                            <div class="flex items-center gap-3">
                                <span class="flex h-9 w-9 items-center justify-center rounded-lg bg-primary-50 text-primary-600"><x-heroicon-o-truck class="h-5 w-5" /></span>
                                <span class="text-sm font-medium text-gray-800" x-text="s.name"></span>
                            </div>
                        </td>
                        <td class="px-4 py-1 text-sm text-gray-500" x-text="s.phone || '—'"></td>
                        <td class="px-4 py-1 text-sm text-gray-500" x-text="s.address || '—'"></td>
                        ...
                        <td class="px-4 py-1">
                            <div class="flex items-center justify-end gap-1">
                                <a :href="'{{ url('suppliers') }}/' + s.id + '/edit'" class="rounded-md p-1.5 text-gray-400 transition hover:bg-primary-50 hover:text-primary-600" title="Edit"><x-heroicon-o-pencil-square class="h-4 w-4" /></a>
        ...
        <x-ui.modal title="Hapus Pemasok" size="sm" show="showDelete">
            <p class="text-sm text-gray-600">Yakin menghapus pemasok <span class="font-semibold" x-text="deleting?.name"></span>?</p>
            <x-slot:footer>
