<x-layouts.app title="Kategori">
        ...
        x-data="{
            ...
            async load() {
                ...
                try {
                    ...
                    const res = await window.api.get('/api/categories?' + params.toString());
            ...
            async doDelete() {
                try {
                    const res = await window.api.delete('/api/categories/' + this.deleting.id);
                    this.$store.toasts.success(res.message || 'Kategori dihapus');
    ...
    >
        <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="sm:w-80"><x-ui.search-input placeholder="Cari nama kategori..." model="search" /></div>
            <x-ui.button :href="route('categories.create')" icon="plus">Tambah Kategori</x-ui.button>
        </div>
        ...
        <div x-show="!loading && !error && items.length > 0">
            <x-ui.table>
                <x-slot:head>
                    ...
                    <x-ui.th>Deskripsi</x-ui.th>
                    <x-ui.th align="center">Jumlah Produk</x-ui.th>
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-1">
                            <div class="flex items-center gap-3">
                                <span class="flex h-9 w-9 items-center justify-center rounded-lg bg-primary-50 text-primary-600"><x-heroicon-o-rectangle-stack class="h-5 w-5" /></span>
                                <span class="text-sm font-medium text-gray-800" x-text="c.name"></span>
                            </div>
                        </td>
                        <td class="px-4 py-1 text-sm text-gray-500" x-text="c.description || '—'"></td>
                        <td class="px-4 py-1 text-center">
                            <span class="inline-flex min-w-9 justify-center rounded-md bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-600" x-text="c.products_count ?? 0"></span>
                        </td>
                        <td class="px-4 py-1">
                            <div class="flex items-center justify-end gap-1">
                                <a :href="'{{ url('categories') }}/' + c.id + '/edit'" class="rounded-md p-1.5 text-gray-400 transition hover:bg-primary-50 hover:text-primary-600" title="Edit"><x-heroicon-o-pencil-square class="h-4 w-4" /></a>
        ...
        <x-ui.modal title="Hapus Kategori" size="sm" show="showDelete">
            <p class="text-sm text-gray-600">
                Hapus kategori <span class="font-semibold" x-text="deleting?.name"></span>?
                <span x-show="deleting?.products_count > 0">
                    <span x-text="deleting?.products_count"></span> produk akan menjadi tanpa kategori.
                </span>
            </p>
            <x-slot:footer>
