{{-- Grid produk bahan servis. Membaca scope Alpine dari halaman create. Harga = harga modal. --}}
        ...
        <div x-show="viewMode === 'card'" class="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">
            <template x-for="p in products" :key="p.id">
                <button type="button" @click="addProduct(p)" :disabled="p.stock <= 0"
                    ...
                    <span class="mt-1 text-sm font-semibold text-primary-600" x-text="rupiah(p.purchase_price)"></span>
                    <span class="mt-0.5 text-[11px]" :class="p.stock <= p.min_stock ? 'text-danger-500' : 'text-gray-400'">
                        Modal &middot; Stok: <span x-text="p.stock"></span>
                    </span>
                </button>
            </template>
        </div>
        ...
        <div x-show="viewMode === 'list'" class="space-y-2">
            <template x-for="p in products" :key="p.id">
                <button type="button" @click="addProduct(p)" :disabled="p.stock <= 0"
                    ...
                    <span class="flex-shrink-0 text-sm font-semibold text-primary-600" x-text="rupiah(p.purchase_price)"></span>
